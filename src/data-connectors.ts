...
export class BenzeneReport extends DataConnector {
  private readonly urlBase =
    'https://fenceline-ambient-benzene-apims.azure-api.net/fenceline-ambient-benzene/';
  private facilities: BenzeneReportFacility[] = [];

  initializeFacilities() {
    return (
      makeRequest('GET', this.urlBase + 'facilities') as Promise<string>
  ...
  getDataSources(): Promise<DataSource[]> {
    return this.initializeFacilities().then(() => {
      return this.facilities.map(
        (facility: BenzeneReportFacility, i: number) => {
          const tags = [
